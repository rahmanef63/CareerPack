...
**Guard helpers in `convex/_lib/auth.ts` — use consistently:**
...
**AI actions must pipeline through:** `requireQuota(ctx)` (`_lib/rateLimit.ts`, token bucket 10/min + 100/day) → `sanitizeAIInput()` → `wrapUserInput()` before hitting the OpenAI-compatible proxy.
