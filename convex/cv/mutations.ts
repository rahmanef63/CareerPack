...
// ---------------------------------------------------------------------------
// MCP write path (convex/mcp/tools/cv.ts). The read path and set-summary live
// in convex/mcp/data/cv.ts; these four are here because they need
// `cascadeRemoveCV` and `resolveCVId`, which are module-private to this file.
//
// They duplicate the guards above rather than reuse them because an MCP
// request carries no @convex-dev/auth session: `requireUser` throws and
// `requireOwnedDoc` calls it, so identity arrives as an explicit `userId`
// resolved from the bearer token and every document is re-checked here.
// ---------------------------------------------------------------------------

/** Mirrors CV_TEMPLATES in frontend/slices/cv-generator/constants. The public
 *  mutation takes any string; an MCP model inventing "professional" would
 *  render a blank CV, so the MCP path pins it to what exists. */
const CV_TEMPLATE_WHITELIST = new Set(["classic", "modern", "minimal"]);
...
/** Resolve the target CV, defaulting to the user's default one, and prove
 *  ownership. "not found" for a foreign id — never "forbidden", which would
 *  confirm the id exists. */
async function mcpTargetCV(
  ctx: MutationCtx,
  userId: Id<"users">,
  cvId: Id<"cvs"> | undefined,
) {
  const resolved = cvId ?? (await resolveCVId(ctx, userId));
  if (!resolved) throw new Error("Belum ada CV — buat CV dulu.");
  const cv = await ctx.db.get(resolved);
  if (!cv || cv.userId !== userId) throw new Error("CV tidak ditemukan");
  return cv;
}

export const mcpCreate = internalMutation({
  args: { userId: v.id("users"), title: v.string(), template: v.string() },
  handler: async (ctx, args) => {
    await mcpQuota(ctx, args.userId);
    if (!CV_TEMPLATE_WHITELIST.has(args.template)) {
    ...
    const cvId = await ctx.db.insert("cvs", {
      userId: args.userId,
      title: assertShortText(args.title, 120, "Judul CV"),
      template: args.template,
      personalInfo: {
        fullName: profile?.fullName || user?.name || "",
        email: user?.email || "",
    ...
    return { cv_id: cvId, title: args.title, template: args.template };
  },
});
...
export const mcpDelete = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await cascadeRemoveCV(ctx, args.userId, args.cvId);
    await ctx.db.delete(args.cvId);
    return { deleted: true, title: cv.title };
  },
});

export const mcpAppendExperience = internalMutation({
  args: {
    userId: v.id("users"),
    cvId: v.optional(v.id("cvs")),
    company: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    // The UI mutation stores these uncapped; anything an MCP token writes
    // goes through the same caps `updateCV` enforces on a manual edit.
    const entry = {
      id: `exp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      company: assertShortText(args.company, 120, "Perusahaan"),
      position: assertShortText(args.position, 120, "Posisi"),
      startDate: assertShortText(args.startDate, 40, "Tanggal mulai"),
      endDate: capLen("Tanggal selesai", args.endDate, 40),
      current: args.current ?? !args.endDate,
      description: assertShortText(args.description ?? "", 2000, "Deskripsi"),
      achievements: capStringArray(args.achievements ?? [], {
        maxEntries: CV_ENTRY_MAX,
        maxLen: 300,
        field: "Pencapaian",
        entryField: "Pencapaian",
    ...
    assertCount([...cv.experience, entry], "Pengalaman");

    await ctx.db.patch(cv._id, { experience: [...cv.experience, entry] });
    return { cv_id: cv._id, entry_id: entry.id };
  },
});

export const mcpAppendSkills = internalMutation({
  args: {
    userId: v.id("users"),
    cvId: v.optional(v.id("cvs")),
    skills: v.array(v.string()),
  ...
  handler: async (ctx, args) => {
    ...
    const added = capStringArray(args.skills, {
      maxEntries: CV_ENTRY_MAX,
      maxLen: 60,
      field: "Skill",
      entryField: "Skill",
    ...
    if (added.length === 0) return { cv_id: cv._id, added: 0, skipped_duplicates: true };
    assertCount([...cv.skills, ...added], "Skill");

    await ctx.db.patch(cv._id, { skills: [...cv.skills, ...added] });
    return { cv_id: cv._id, added: added.length, names: added.map((s) => s.name) };
  },
});
