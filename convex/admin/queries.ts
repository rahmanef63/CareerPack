    ...
    return await Promise.all(
      users.map(async (u) => {
        ...
        return {
          ...
          fullName: p?.fullName ?? "",
          location: p?.location ?? "",
          // Empty = never observed, NOT "no country". Deliberately not
          // defaulted to `location`: that field is user-typed and would
          // print a self-declared city as if it were a measured signal.
