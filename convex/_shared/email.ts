 ...
 * - Otherwise, logs the message to backend stdout AND returns `ok: false`.
 *   The log is the dev affordance (a copy-pasteable reset link); the failure
 *   result is the honest answer, since nothing left the building.
 *
...
export type SendResult =
  /** Handed to Resend, or deliberately withheld (recipient unsubscribed). */
  | { ok: true; provider: "resend" | "suppressed" }
  /** Nothing was delivered. `email_unconfigured` = no RESEND_API_KEY. */
  | { ok: false; reason: string };

/**
 * Is outbound mail actually wired up in this deployment? Exposed so
 * `/api/health` can report readiness without anyone needing to trigger a
 * real password reset to find out — see convex/health.ts.
 *
 * Only checks for the key's presence. Whether the sender DOMAIN is verified
 * in Resend is not knowable from here; that shows up as a `resend_403` at
 * send time.
 */
...
export async function sendEmail(
  ...
  if (!apiKey) {
    // Still print the body — this is how you get a working reset link in
    // local dev. But report FAILURE, because nothing was delivered.
    //
    // This used to return ok:true, which made every caller's `if (!result.ok)`
    // branch unreachable whenever the key was missing: a production deploy
    // with no RESEND_API_KEY logged "sent" for every password reset while the
    // user waited forever for mail that was never even attempted. A mailer
    // that reports success without sending is worse than one that is simply
    // down, because nothing anywhere alerts.
    console.error(
      `[email] NOT SENT — RESEND_API_KEY unset. to=${input.to} subject="${input.subject}" tag=${input.tag ?? "n/a"}\n${input.text ?? stripHtml(input.html)}`,
    );
