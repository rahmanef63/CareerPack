...
/** Per-section import ceiling. A malformed AI extraction can hand us a
 *  20k-element array; inserting it all in one transaction fails with an
 *  opaque error AFTER the client already reported success. Mirrors the
 *  MAX_SKILLS / MAX_DOCS caps on the sibling seed paths. */
const MAX_PER_SECTION = 200;

function capSection<T>(rows: T[], label: string, warnings: string[]): T[] {
  if (rows.length <= MAX_PER_SECTION) return rows;
  warnings.push(
    `${rows.length - MAX_PER_SECTION} ${label} dilewati (maksimal ${MAX_PER_SECTION} per impor).`,
  );
  return rows.slice(0, MAX_PER_SECTION);
}
...
export const quickFill = mutation({
  ...
  handler: async (ctx, args): Promise<QuickFillResult & { batchId: Id<"quickFillBatches"> | null }> => {
    ...
    if (Array.isArray(payload.portfolio)) {
      for (const raw of capSection(payload.portfolio, "portofolio", result.warnings)) {
    ...
    if (Array.isArray(payload.goals)) {
      for (const raw of capSection(payload.goals, "goal", result.warnings)) {
    ...
    if (Array.isArray(payload.applications)) {
      for (const raw of capSection(payload.applications, "lamaran", result.warnings)) {
    ...
    if (Array.isArray(payload.contacts)) {
      for (const raw of capSection(payload.contacts, "kontak", result.warnings)) {
