...
// The sweep pages itself with a cursor (re-scheduling per page), so the
// cron kicks off page one with no cursor.
crons.weekly(
  "weekly-job-digest",
...
// Daily model-catalog refresh from models.dev, so the admin picker keeps up
// with vendors shipping models. 03:00 UTC (10:00 WIB) is arbitrary but off the
// two jobs above. Idempotent: it overwrites one singleton row, and a bad
// response leaves the previous catalog in place rather than emptying it.
crons.daily(
