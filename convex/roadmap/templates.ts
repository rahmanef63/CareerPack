...
/**
 * MCP (convex/mcp/tools/roadmap.ts). No `userId`: `roadmapTemplates` is the
 * shared catalog and this only ever returns `isPublic` rows, so there is
 * nothing user-scoped to check. Projected down to what a model needs to pick
 * a slug — `nodes[]` alone is tens of thousands of tokens.
 */
export const mcpListTemplates = internalQuery({
  args: {},
  handler: async (ctx) => {
    const all = await ctx.db
      .query("roadmapTemplates")
    ...
    return {
      items: all
        ...
        .map((t) => ({
          ...
          description: t.description,
          tags: t.tags,
          skill_count: t.nodes.length,
          total_hours: t.nodes.reduce((s, n) => s + n.estimatedHours, 0),
        })),
    };
  },
});
...
/**
 * Non-throwing visibility gate. `adminToggleTemplatePublic` exists to
 * unpublish a template, but these two getters returned the full row —
 * nodes[] and authorName included — to anyone holding the slug, so
 * moderation hid nothing. Author + admin keep access.
 */
async function canSeeTemplate(
  ctx: QueryCtx,
  tpl: Doc<"roadmapTemplates">,
): Promise<boolean> {
  if (tpl.isPublic) return true;
  const userId = await optionalUser(ctx);
  if (!userId) return false;
  if (tpl.authorId === userId) return true;
