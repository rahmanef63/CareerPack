...
/** Only feed-fetched rows are prunable. Scoping the scan to these
 *  sources (instead of windowing by_posted globally and skipping
 *  never-prune rows afterwards) is what stops the batch from being
 *  eaten by the user-paste / seed rows that pile up at the head of the
 *  ascending postedAt index — once 200 of those sit there, every run
 *  deleted 0 forever. */
const PRUNABLE_SOURCES = ["remoteok", "wwr"] as const;
...
export const pruneOldJobs = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    let scanned = 0;
    let deleted = 0;
    for (const source of PRUNABLE_SOURCES) {
      const stale = await ctx.db
        .query("jobListings")
        .withIndex("by_source_posted", (q) =>
          q.eq("source", source).lt("postedAt", cutoff),
        )
        .take(PRUNE_BATCH);
    ...
    console.log(`[prune] scanned=${scanned} deleted=${deleted}`);
