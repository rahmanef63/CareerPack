...
describe("buildBrandingPayload", () => {
  ...
  it("toggles override has-flags even when data is present", () => {
    ...
    // Toggled-off sections must not be serialised at all — getBySlug is
    // unauthenticated, so shipping the data and hiding it is a leak.
    ...
    expect(out.about).toEqual({ bio: "", summary: "" });
    // fullCv() carries projects and portfolio is empty, so this array is
    // CV-sourced — and CV projects have no per-item visibility flag, making
    // showProjects the only gate on them.
