...
describe("deleteMyAccount", () => {
  it("erases the caller and nothing that belongs to anyone else", async () => {
    const t = setup();
    const mine = await insertUser(t);
    const theirs: Id<"users"> = await t.run((ctx) =>
      ctx.db.insert("users", { email: "other@x.com" }),
    );
    for (const userId of [mine, theirs]) {
      ...
      await t.run((ctx) =>
        ctx.db.insert("cvs", {
          ...
          title: "CV",
          template: "modern",
          personalInfo: { fullName: "X", email: "x@x.com", phone: "", location: "", summary: "" },
          experience: [], education: [], skills: [], certifications: [],
          languages: [], projects: [], isDefault: true,
        }),
      );
    }

    await t.withIdentity(identity(mine)).mutation(api.profile.mutations.deleteMyAccount, {});

    const left = await t.run(async (ctx) => ({
      users: (await ctx.db.query("users").collect()).map((u) => u._id),
      profiles: (await ctx.db.query("userProfiles").collect()).map((p) => p.userId),
      cvs: (await ctx.db.query("cvs").collect()).map((c) => c.userId),
    }));
    expect(left).toEqual({ users: [theirs], profiles: [theirs], cvs: [theirs] });
  });

  it("refuses when the caller is the last admin", async () => {
    const t = setup();
    const userId = await insertUser(t);
    const asUser = t.withIdentity(identity(userId));
    await asUser.mutation(api.profile.mutations.createOrUpdateProfile, validProfileArgs());
    // `t.run` hands back an un-parameterised ctx, so named indexes don't
    // typecheck here — this test only ever has the one profile row.
    await t.run(async (ctx) => {
      const [profile] = await ctx.db.query("userProfiles").collect();
      await ctx.db.patch(profile._id, { role: "admin" });
    });

    await expect(
      asUser.mutation(api.profile.mutations.deleteMyAccount, {}),
    ).rejects.toThrow("satu-satunya admin");
  ...
  it("rejects an unauthenticated caller", async () => {
    const t = setup();
    await expect(
      t.mutation(api.profile.mutations.deleteMyAccount, {}),
