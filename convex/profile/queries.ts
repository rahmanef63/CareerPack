...
export const listIndexableSlugs = query({
  args: {},
  handler: async (ctx) => {
    // Unauthenticated, called from /sitemap.xml. `.collect()` here
    // deserialised every profile row (skills[]/interests[]/publicBlocks[])
    // just to extract a handful of slugs — one crawler hit was enough to
    // push it toward the read budget and 500 the public sitemap.
    const profiles = await ctx.db
      .query("userProfiles")
      .withIndex("by_public_index", (q) =>
        q.eq("publicEnabled", true).eq("publicAllowIndex", true),
      )
      .take(MAX_SITEMAP_SLUGS);
    return profiles
      .map((p) => p.publicSlug)
