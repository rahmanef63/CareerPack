...
export function buildBrandingPayload({
  ...
  return {
    ...
    // Gate the DATA, not just the `has` map — `getBySlug` is
    // unauthenticated, so a section the user switched off must never be
    // serialised over the wire (hiding it client-side is not privacy).
    about: {
      bio: t.showBio ? bio : "",
      summary: t.showBio ? cvSummary : "",
    },
    skills: t.showSkills ? skillsMerged : [],
    ...
    // Portfolio rows arrive already filtered by per-item brandingShow, but
    // cvProjects have no per-item control at all — so the section toggle is
    // the ONLY thing standing between them and an unauthenticated getBySlug.
    projects: t.showProjects ? projectCards : [],
