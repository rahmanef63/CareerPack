...
import { isEncryptedCred } from "../_shared/aiCrypto";

function maskKey(key: string): string {
  if (!key) return "";
  // An encrypted key has no meaningful first/last four — the untouched mask
  // would show "encv" and four characters of base64, which reads like a
  // corrupted key and sends the user re-pasting a key that works fine.
  if (isEncryptedCred(key)) return "••••••••";
...
export const listAIProviders = query({
  args: {},
  handler: async (ctx) => {
    const base = listProvidersPublic();
    // Merge in whatever the models.dev refresh last cached (ai/catalog.ts).
    // Additive and union-ed, never a replacement: the cached list is IDs only,
    // so a provider missing from it — or an empty table before the first cron
    // tick — falls through to the hand-maintained list rather than rendering an
    // empty picker. Keeping the hand-maintained entries in the union also means
    // the model an admin already selected never vanishes from the dropdown
    // because upstream retired it.
    const cached = await ctx.db.query("modelCatalog").first();
    if (!cached) return base;
    const byId = new Map(cached.providers.map((p) => [p.id, p.models]));
    return base.map((p) => {
