...
export const setMyAISettings = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // New keys get encrypted; an untouched `existing.apiKey` is copied through
    // byte-for-byte. That is what keeps this from being a migration — a row
    // written before AI_CRED_SECRET existed stays plaintext until its owner
    // next types a key, and `decryptCred` reads it either way.
    const apiKey = rawKey ? await maybeEncryptCred(rawKey) : existing?.apiKey || "";
...
export const setGlobalAISettings = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // field blank must NOT re-encrypt the live plaintext global key: rewriting
    // it takes every AI feature down at once if AI_CRED_SECRET is wrong.
    const apiKey = rawKey ? await maybeEncryptCred(rawKey) : existing?.apiKey || "";
...
export const upsertChatSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Cap check + eviction via two bounded index reads. `.collect()` here
    // used to materialise every session INCLUDING its full `messages`
    // array just to read `.length` and the last element — tens of MB of
    // transaction reads that eventually make new chats impossible.
    // Ascending on by_user_updated ⇒ cap[0] is the eviction candidate.
    const cap = await ctx.db
      .query("chatConversations")
      .withIndex("by_user_updated", (q) => q.eq("userId", userId))
      .take(MAX_SESSIONS_PER_USER);
    if (cap.length >= MAX_SESSIONS_PER_USER) {
      await ctx.db.delete(cap[0]._id);
    }
...
export const deleteAllChatSessions = mutation({
  ...
  handler: async (ctx) => {
    const userId = await requireUser(ctx);
    // Page instead of `.collect()` — every row carries its full
    // `messages` array, so materialising them all at once is what makes
    // this mutation fall over on a heavy account.
    for (;;) {
