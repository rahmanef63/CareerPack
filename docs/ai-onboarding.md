...
│  ├─ lib/dashboardRegistry.ts     SSOT registry for dashboard routing + nav
│  ├─ components/layout/*          Sidebar, BottomNav, Header, NavUser
│  ├─ components/files/*           FileUpload (drag-drop + crop + WebP)
│  └─ components/ui/*              shadcn primitives
└─ slices/                         22 feature slices (kebab-case)
convex/
├─ schema.ts                       orchestrator — 45 tables + authTables
├─ _shared/auth.ts                 requireUser, requireOwnedDoc, requireAdmin
├─ _shared/rateLimit.ts            Token bucket (10/min + 100/day)
├─ _shared/sanitize.ts             sanitizeAIInput, wrapUserInput
├─ files/                          Convex blob store (generate/save/get/list/delete)
└─ <domain>/                       schema fragment + queries/mutations/actions
```
...
- **Dashboard routing**: every dashboard page resolves through a
  **single catch-all** `app/(dashboard)/dashboard/[[...slug]]/page.tsx`
  looking up a view in `DASHBOARD_VIEWS`. Adding a page = **1 edit**
  (one entry in `shared/lib/dashboardRegistry.ts`; routing + nav are
  derived from it). See **G1**.
...
- **Auth cryptography**: PBKDF2-SHA256 100k iter custom (NOT Scrypt —
  ...
  swap it.
- **Login-or-register single call**: `useAuth.login` POSTs the
  `/api/auth/check-email` httpAction (IP-gated 30/hr; the public
  `userExistsByEmail` query was deleted for email enumeration) then
  signs in or signs up in one shot. Password
...
- 22 slices (CV builder, AI agent, calendar, career dashboard, skill
  roadmap, document checklist, mock interview, financial calculator,
  matcher, networking, portfolio, library, personal branding, database,
  notifications, help, dashboard home, settings, ai-settings, hero,
  auth, admin panel)
...
Before claiming a task complete, run these in order. The pre-push hook
runs them on every push (GitHub Actions is manual-dispatch only).
