...
| `og-image.png` (root) | 1200×630 | **Legacy** OpenGraph card. Nothing in the app references it any more — `app/layout.tsx` points `openGraph`/`twitter` at `/og/og-light.jpg`. The generator still emits it so links shared before the brand refresh keep resolving. |
| `og/og-light.jpg` | 1200×630 | Current OpenGraph + Twitter share card |

**Regeneration** — `node scripts/generate-pwa-assets.mjs` rebuilds this
whole matrix from the committed brand assets (`frontend/public/brand/*`
+ `frontend/public/og/og-light.jpg`) via `sharp`. It is **not** sourced
from a hand-coded SVG any more; the only thing still drawn inline is
the `screenshots/{narrow,wide}.png` pair, because no real captures
exist yet. Output is committed so production serves static files.
