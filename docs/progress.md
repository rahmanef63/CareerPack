...
Last updated: 2026-06-11 (sesi ke-2).

## Recent batches

- 2026-06-15 — **Version-control script monitoring + self-heal host (Finding #5).** Script health-watch + self-heal yang mendeteksi & memulihkan insiden 2026-06-11 sebelumnya hanya hidup di host (tak ter-track git) — kini direkonstruksi dari postmortem + pola `backup.sh` ke `backend/convex-self-hosted/ops/health-watch.sh` (4 probe: frontend 200, convex-api `/version` 200, site-health `/api/health` ok:true, container docker health; **self-heal** `docker compose -p careerpack-convex-8gdbpk up -d` saat container hilang; cek kesegaran backup warn jika arsip > ~25 jam; alert hook configurable via env, default `~/.config/health-watch.alert`) + `ops/install-cron.sh` (installer crontab idempotent: backup `0 3` harian + health-watch `*/3` menit, tanpa duplikat). Keduanya `set -euo pipefail`, header gaya `backup.sh`, tanpa secret, lulus `bash -n`. `docs/deployment.md` §5b "Monitoring & Self-Heal" baru mendokumentasikan crontab + alert wiring.
- 2026-06-11 (sesi 2c) — **Insiden prod + hardening VPS.** Backend Convex prod DOWN (api/site.careerpack.org → Traefik 404, WS sync gagal): container compose hilang — image pinned ter-prune dari host + re-pull kena ghcr.io 502. Volume data (227 MB) utuh. Recovery: re-pull + `compose up -d` reattach volume/network lama, lalu `pnpm backend:deploy` (backend ternyata tertinggal dari HEAD — deploy menambah index `outcomeEvents.by_from_to` dkk). **Hardening terpasang di host:** (1) `restart: unless-stopped` di-persist ke compose disk **dan** `composeFile` di DB Dokploy (Dokploy menyimpan compose user di DB + inject label Traefik saat deploy — edit disk saja hilang saat redeploy); (2) CareerPack didaftarkan ke host `health-watch.sh` cron 3-menit (4 check, deteksi ≤6 mnt + push alert) + blok **self-heal** `compose up -d` saat container backend hilang; (3) **backup cron harian 03:00 LIVE** (root crontab, `VOLUME_NAME` di-pin, arsip pertama 121 MB terverifikasi berisi `db.sqlite3` + blobs) — menutup backlog #5 yang tertunda sejak 2026-05-07. **Bug fix:** auto-detect `backup.sh` terbukti berbahaya di host multi-project (broad grep + `head -1` match volume project lain) → kini fail-loud kecuali match tepat satu. Detail: [`progress/2026-06-11-vps-incident-and-hardening.md`](./progress/2026-06-11-vps-incident-and-hardening.md). Sisa manual: provider auto-snapshot toggle + 1× recovery test.
- 2026-06-11 (sesi 2b) — **Eksekusi audit kode menyeluruh — 3 kritis + ~25 moderat fixed, gate hijau (294 vitest, 27 file).** Audit 5-agent paralel + verifikasi manual → [`docs/audit/2026-06-11-bugs-and-improvements.md`](./audit/2026-06-11-bugs-and-improvements.md), lalu semua temuan kritis dieksekusi: **(C1, kritis/config)** `resolveAI()` ter-copy 4× dengan urutan resolusi terpotong (skip admin-global key + per-user override) — admin set 1 global key: chat jalan tapi CV translate/matcher/planner "belum dikonfigurasi". Dikonsolidasi ke `_shared/aiResolve.ts`, 5 call site rewire. **(C2, kritis/XSS)** stored XSS di halaman publik `/<slug>`: sanitizer HTML blocklist `on*=` menuntut whitespace sebelum `on` → `<a href="x"/onmouseover=…>` lolos & dirender verbatim (tag `a` di-allowlist). Rewrite ke **allowlist-atribut** + re-check protokol href (termasuk unquoted) + 12 test regresi bypass. **(C3, kritis/GDPR)** `cascadeDeleteUser` melewatkan 4 tabel ber-`userId` (`truthAtoms`+blob proof, `outcomeEvents`, `careerQuests`, `aiUserModelOverrides`). **Backend:** storage-IDOR guard `assertOwnedStorages` (portfolio cover/media + avatar CV), dedup `saveFile` per-tenant, HMAC reset fail-closed (fallback konstanta publik dihapus), `createQuest` mirror bound validator (NaN/dup-id), caps input contacts+calendar (`_shared/validate.ts`), NaN guard roadmap, refund quota plan 200-junk, prompt-injection profil di-sanitize, `authCheckEmail` pakai index `email`, dead `LOGIN_RATE_LIMIT` dihapus. **Frontend:** crash mock-interview (index basi saat ganti kategori), input checklist "beku" (controlled by snapshot basi + mutation per keystroke → state lokal + flush on blur), networking mutations tanpa error handling → `withMutationToast`, AI console (`finally` anti spinner-nyangkut + `thinkingId` per-sesi + toast), race OpenRouterModelPicker, seed-on-open PortfolioForm, filter per-tab checklist. **UX/a11y:** `bg-muted/50/50` typo (4 file), confirm-dialog hapus (calendar/roadmap), label `htmlFor`/aria form CV + checklist keyboard-operable, tap target ≥36px + aksi visible di touch, kanban breakpoint tablet, i18n EN→ID. **Test baru:** sanitizer XSS (12), gamification (15), atoms hash (11) → 294 total. **False positive dicegat:** `requireQuota` "duplikat" ternyata bucket kuota berbeda per domain — tidak digabung. Commits: `e7a8299` (convex), `3f0344a` (ui), `cc18371` (docs).
- 2026-06-11 (sesi 2a) — **Google OAuth diagnosis.** `signIn("google")` gagal di prod: kode 100% benar (provider terdaftar, `addHttpRoutes`, tombol login) — yang hilang murni konfigurasi ops: `AUTH_GOOGLE_ID`/`AUTH_GOOGLE_SECRET`/`SITE_URL` tidak pernah di-set di backend + OAuth client Google Console dengan redirect URI `.site` origin (`<CONVEX_SITE_ORIGIN>/api/auth/callback/google`, BUKAN frontend/`:3210`). Didokumentasikan di `convex.env.example` (blok Google OAuth) + `docs/auth.md` §1b/§5/§6 (setup + 4 baris troubleshooting). Eksekusi setup-nya menunggu akses Google Console.
- 2026-06-11 — Bug-fix batch (post-audit hunt) + offline UX + coverage ratchet. Fixed 5 real bugs found by an adversarial bug-hunt: **(1, critical/IDOR)** `matcher.queries._getOwnedCV` did `db.get(cvId)` with NO ownership check — any user could run an ATS scan against another user's CV and persist its keywords into their own `atsScans` (data leak). Now takes `userId` + returns null on mismatch, caller passes `userId`; guarded by a new convex-test case. **(3, critical)** calendar `reminders.parseEventStart` parsed event wall-clock as UTC (`…Z`) while the ICS exporter treats the same fields as floating WIB — every reminder fired 7h off for ID users. Now anchors to `APP_UTC_OFFSET = "+07:00"`; new `reminders.test.ts`. **(6+7, medium)** `getSalaryInsights` mixed IDR+USD midpoints into one percentile distribution and used a floor-biased index (p75 of 2 values returned the min). Aggregation extracted to pure `matcher/salaryStats.ts` (percentiles from the dominant currency only, `Math.round` index); new `salaryStats.test.ts`. **(8, low)** `formatShortIDR` emitted "Rp 1000,0 rb" at the rounding boundary + had a stale docstring — fixed with a 0.99995 unit-promotion margin; new `formatCurrency.test.ts`. **False positive caught:** the bug-hunt also flagged `engine.graph.edgeKey`/`calibrator` as a "dead calibration loop" (`indexOf("")`) — but the bytes were `indexOf("\x01")`: an **invisible 0x01 control char** was the delimiter (1 in `graph/lib.ts`, 2 in `calibrator.ts`). The loop actually works; the control chars were swapped for a visible `|` (behaviour-identical, neither char appears in kebab slugs) so the code is grep-able and a future edit can't silently strip the delimiter. New `edgeKey` round-trip/collision tests. **DP note:** `cohortStatsDP` gating only on `apply` count is defensible (Laplace already provides the formal guarantee) — documented, not changed. **UX:** new `useOnlineStatus` hook + global `OfflineBanner` (shown only when `navigator.onLine` is false) so users see *why* a mutation fails offline. Coverage floor ratcheted 23→26 after the new tests. Full gate green: typecheck + lint + 257 vitest (was 235) + coverage 26.77% ≥ threshold. Detail: [`docs/audit/2026-06-11-full-project-audit.md`](./audit/2026-06-11-full-project-audit.md) §11.4.
- 2026-06-11 — Full project audit + docs reconciliation + pre-push quality gate. New [`docs/audit/2026-06-11-full-project-audit.md`](./audit/2026-06-11-full-project-audit.md): progress timeline terverifikasi, maturitas per slice, kritik 3 perspektif (user / investor / QA), rencana 30 hari launch-enablement. **Reconciliation** (docs drift vs realita): CLAUDE.md §CI kini mencerminkan switch `workflow_dispatch`-only 2026-05-14 (`b678c58`; trigger asli tersimpan sebagai `*.yml.bak`), §Providers tree disesuaikan dengan `shared/providers/Providers.tsx` (InstallChip → `InstallSidebarButton`, +GA4), §Auth login-flow disesuaikan dengan `/api/auth/check-email` (query `userExistsByEmail` sudah dihapus 2026-05-07), dan §BSDL ditandai legacy (kitab tore down BSDL 2026-05-16; `.kitab.json` dihapus di `75eabcc`). **Hardening**: `scripts/pre-push.sh` kini menjalankan `pnpm typecheck` + `vitest run` sebelum step deploy Convex (bypass `SKIP_PUSH_CHECKS=1`) — dengan Actions manual-only, hook ini gate otomatis terakhir sebelum main. `vitest.config.ts` dapat coverage `thresholds` (floor anti-regresi di bawah baseline 2026-06-11: 24.81/25.35/19.70/24.72) + script `pnpm test:coverage`. **Fix**: `files.listMyFiles` usage map diperluas dari portfolio-only ke avatar CV (`personalInfo.avatarStorageId`) + foto profil (`userProfiles.avatarStorageId`) — warning delete di Library kini mencakup semua domain yang pin storageId. **Fix lint**: `pnpm lint` ternyata merah di main (`app/[slug]/opengraph-image.tsx` `no-img-element` — tak terdeteksi karena pre-commit hanya lint file staged + CI manual); ditutup dengan disable berjustifikasi (next/og `ImageResponse` tidak mendukung `next/image`). Full gate green: typecheck + lint + 235 vitest + coverage thresholds.
- 2026-05-28 — Convex auth-guard consistency + first guard regression tests. Migrated the 6 straggler inline-guard sites onto the shared helpers in `convex/_shared/auth.ts`: `ai/mutations.ts` `setMyAISettings` / `toggleAIEnabled` / `clearMyAISettings` → `requireUser`; `ai/queries.ts` `getMyQuota` / `getMyAISettings` → `optionalUser`; `seed.ts` `seedForCurrentUser` → `requireUser`. Behaviour-preserving (helpers throw the same `"Tidak terautentikasi"` / return the same `null`), removes the unused `getAuthUserId` imports, and closes the QA report §2.3 + Recommendation #1 (`aiSettings` inconsistency). New `convex/_shared/auth.test.ts` is the first rule-level coverage of the security guards (QA report §5 gap): 20 cases over `requireUser`, `optionalUser`, `requireOwnedDoc` (incl. the R6 enum-safe invariant — owned-by-other and missing both throw the identical `"… tidak ditemukan"`), `requireAdmin` (role path + super-admin email bypass + denial), `requireSuperAdmin`, and `isSuperAdminCaller`. Done by mocking `@convex-dev/auth/server#getAuthUserId` + a minimal fake `ctx.db`. Then added `convex-test` (dev-only) + `convex/guards.integration.test.ts` — 8 end-to-end cases driving the same guards through *real* Convex functions: `requireUser` on `clearMyAISettings`, `optionalUser` SSR-safety on `getMyQuota`, `requireAdmin` on `clearGlobalAISettings` (role path), cross-user chat-session isolation, and `enforceRateLimit` budget+overflow against a live transaction — closing QA report Recommendation #2 (Convex test-client suites). Also fixed config rot in `vitest.config.ts`: coverage `include` glob `convex/_lib/**` → `convex/_shared/**` (stale since the 2026-04-25 restructure renamed `_lib`). Full gate green: typecheck + lint clean, 235 vitest cases (was 207), production build OK.
...
5. ~~Backup cron eksekusi~~ ✓ closed 2026-06-11 — cron harian 03:00 terpasang di root crontab VPS (`VOLUME_NAME` pinned), arsip pertama 121 MB terverifikasi. Sisa: provider snapshot + 1× recovery test.
...
- ~~`/rr-send document-checklist`~~ — obsolete 2026-05-16: kitab
  tore down BSDL; `.kitab.json` dihapus (`75eabcc`). Sharing slice
  kini manual `cp -r` + adapt + commit di repo target.
- vector-search adoption (manual copy dari rr — BSDL pipeline gone) —
  masih gated on Convex vector-index validation on pinned Dokploy
  image.
- Convex deploy reliability — Phase 2 deploy 1x gagal (transient,
  next deploy success). Investigate kalau berulang.
- Truly out-of-code: ~~backup VPS install~~ (✓ 2026-06-11), non-tech
  roadmap domain expert content, Google OAuth env + Console setup
  (lihat docs/auth.md §1b), provider auto-snapshot, 1× recovery test.
