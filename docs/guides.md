...
**One edit + a slice folder.** That's the whole flow.
...
**2. Register it** — one entry in `frontend/shared/lib/dashboardRegistry.ts`:
...
const REGISTRY: readonly RegistryEntry[] = [
  // ...
  { slug: "my-feature", view: MY_FEATURE, nav: { placement: "more", id: "my-feature", label: "My Feature", icon: SomeIcon, hue: "from-sky-400 to-sky-600" } },
];
```

`DASHBOARD_VIEWS`, `PRIMARY_NAV`, dan `MORE_APPS` di-derive dari array itu —
`dashboardRoutes.tsx` + `navConfig.ts` cuma re-export shim, jangan tambah data
di sana (dulu dua tempat, dan slug-nya rutin drift).
...
```ts
import type * as myFeature_mutations from "../myFeature/mutations.js";
// ...
declare const fullApi: ApiFromModules<{
  // ...
  "myFeature/mutations": typeof myFeature_mutations;
}>;
```
...
Schema additions go in `convex/<domain>/schema.ts` (exported as `<domain>Tables`,
lalu di-spread di root `convex/schema.ts`) with `v.optional(...)` unless the
