...
## R16. Close every mutation / auth action with user feedback

- User-initiated mutations fire a sonner `toast.success` / `toast.error`
  on resolution. No silent successes, no console-only errors.
- `toast.promise(p, { loading, success, error })` for long actions
  (>800 ms perceived wait) — AI proxy calls, PDF export, batch import.
- Registration auto-signs-in via `signIn("password", { flow: "signUp" })`
...
## R17. No localStorage for user data — Convex is SSOT

- localStorage is only for **ephemeral UI preferences** (dropdown-open
  state, last-viewed tab, a temporary filter). Anything a user would
  expect to still be there on a different device belongs in Convex.
- The four standard persistence patterns are documented in
  `docs/guides.md` G13: frontend-owns-template, optimistic+rollback,
  debounced-upsert, and one-shot-migration.
- When migrating a feature off localStorage, use a one-shot migration
  flag key (`careerpack_<feature>_migrated_v1=1`) and clear the legacy
  key afterwards. Don't leave the legacy read path alive indefinitely.
- Queries return `undefined` while loading, `null` for unauth/empty.
  Hydration effects MUST guard with `useRef(false)` to avoid double-
  seed on query refetches.
