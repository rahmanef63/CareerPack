...
convexAuth({
  providers: [
    ...
    Anonymous,
    // Google hanya di-register kalau AUTH_GOOGLE_ID + AUTH_GOOGLE_SECRET
    // dua-duanya ada; tanpa itu tombolnya 500 di callback.
...
1. `fetch` POST ke httpAction `/api/auth/check-email` (rate-limit per-IP 30/jam,
   `convex/authCheckEmail.ts`) — cek existing. Query publik `userExistsByEmail`
   sudah **dihapus** 2026-05-07 karena siapa pun bisa enumerasi email terdaftar.
2. Kalau ada → `signIn("password", { …, flow: "signIn" })`
   Kalau tidak → `signIn("password", { …, flow: "signUp", name })` (auto-register)
3. Setelah berhasil, `seedForCurrentUser()` di-trigger (retry 6×200ms nunggu token
   WebSocket nempel). Error di-swallow dengan warn.
...
Frontend pakai `state.user.role` = `userProfile.profile?.role ?? "user"`. Promosi admin lewat env `ADMIN_BOOTSTRAP_EMAILS` (dipakai `seedForCurrentUser` tiap login) atau `api.admin.mutations.updateUserRole`. `/admin` guard pakai field ini.
...
Setiap mutation/query harus guard sendiri via `_shared/auth.ts`:
