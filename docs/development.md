...
`pnpm typecheck` + `pnpm test:coverage` (vitest + cek coverage thresholds) tiap
push, lalu kalau push range menyentuh `convex/**` ia menjalankan
`pnpm backend:deploy` SEBELUM push landing.
