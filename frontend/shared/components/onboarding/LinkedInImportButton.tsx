...
import { AlertCircle, CheckCircle2, FileUp, Linkedin, Loader2, Sparkles, UserPlus } from "lucide-react";
...
export function LinkedInImportButton({
  ...
  // Demo sessions read profile from the localStorage overlay
  // (useDemoOverlay/profile.ts), so `createOrUpdateProfile` lands on the
  // server and nothing in the UI changes. Left ungated, a visitor uploaded a
  // real resume, waited out an AI call we paid for, got "Profil ter-update",
  // and saw the same empty profile — the worst failure shape there is.
  // Gated at the file picker, before the upload, not at Apply.
          ...
          <div className="space-y-4">
            {isDemo && (
              <div className="rounded-lg border-2 border-destructive bg-destructive/10 p-4 text-sm">
                ...
                <p className="mt-1.5 leading-snug text-destructive-text/90">
                  Akun demo menyimpan data <strong>hanya di browser lokal</strong>.
                  Import ini menulis ke server, jadi hasilnya{" "}
                  <strong>tidak akan terlihat</strong> di profil Anda.
                </p>
                <Button
                  size="sm"
                  className="mt-3"
