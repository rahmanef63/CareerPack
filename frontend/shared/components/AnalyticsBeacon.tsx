...
// Cookieless visitor beacon. Fires a `page_view` on every pathname change
// via navigator.sendBeacon → /api/analytics (which resolves geo from the IP
// then discards it). No cookie; `sessionId` is an ephemeral sessionStorage
// random — enough to count unique visits without a stable identity. UTM
// captured from the first URL that carries them and re-sent for the session.
// Skipped on the signed-in dashboard (/dashboard), admin panel (/admin), and
// API routes.

const SESSION_ID_KEY = "careerpack_sid";
const UTM_KEY = "careerpack_utm";
const UTM_FIELDS = [
  "utm_source",
  "utm_medium",
  "utm_campaign",
...
const SKIP = ["/dashboard", "/admin", "/api"];
...
export function AnalyticsBeacon() {
  ...
  // Capture utm_* from the landing URL. Reads window.location.search
  // directly (not useSearchParams) so a page mounting this never gets
  // forced dynamic.
  React.useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    try {
      sessionStorage.setItem(
        UTM_KEY,
        JSON.stringify({ ...readUtm(), ...incoming }),
      );
    } catch {
      /* storage blocked */
    }
