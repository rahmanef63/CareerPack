...
export function DocumentChecklist() {
  ...
  // Convex-backed checklist: frontend owns the canonical template (title +
  // description), server stores completion + notes + expiry per item id.
  // On first visit, seed the server from the template. Subsequent loads
  // merge server state into the template for display.
  ...
  useEffect(() => {
    ...
    if (checklist === undefined) return; // loading
    if (checklist !== null) return; // already exists
  ...
  // Merge server (or demo localStorage) state into the frontend template.
  // Template wins for title/description; per-id state wins for completion.
