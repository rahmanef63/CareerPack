...
export function FinancialCalculator() {
  ...
  // Persisted "plan" snapshot — stores monthlyIncome (as targetSalary)
  // and the target city context. Expenses + savings live in budgetVariables
  // so the plan just captures the salary baseline + relocation target.
  ...
  useEffect(() => {
    ...
    planDebounce.current = setTimeout(() => {
      ...
      savePlan({
        ...
        // Non-fatal; next refetch resyncs.
      });
    }, 800);
    return () => {
      if (planDebounce.current) clearTimeout(planDebounce.current);
