...
import {
  ...
  PortfolioGuide,
  RoadmapGuide,
} from "@/shared/components/illustrations/help";
...
// ponytail: the 4 entries that now have an illustrated GuideCard below were
// deleted instead of duplicated — each asset appears exactly once.
...
interface Guide {
  /** Inline themed illustration — a component, never a file path. */
...
const GUIDES: ReadonlyArray<Guide> = [
  { art: GettingStarted, title: "Mulai dari Nol", href: "/dashboard", description: "Langkah pertama setelah daftar: lengkapi profil dan tentukan target karir." },
  { art: AtsResumeTips, title: "CV ATS-friendly", href: "/dashboard/cv", description: "Format, kata kunci, dan struktur agar CV lolos screening otomatis." },
  { art: InterviewPrep, title: "Persiapan Wawancara", href: "/dashboard/interview", description: "Latihan pertanyaan HR + teknis lengkap dengan timer." },
  { art: OverseasChecklist, title: "Ceklis Kerja Luar Negeri", href: "/dashboard/checklist", description: "Dokumen wajib, apostille, dan syarat per negara tujuan." },
  { art: PortfolioGuide, title: "Bangun Portofolio", href: "/dashboard/portfolio", description: "Susun proyek terbaik jadi halaman publik yang bisa dibagikan." },
  { art: RoadmapGuide, title: "Roadmap Skill", href: "/dashboard/roadmap", description: "Rencana belajar bertahap sesuai kategori karir pilihan." },
];

export function HelpView() {
  return (
    <PageContainer size="md" className="space-y-8 overflow-x-hidden">
...
function GuideCard({ art: Art, title, href, description }: Guide) {
  return (
    <Link
      href={href}
...
function FeedbackSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify.success("Terima kasih — masukan Anda sudah terkirim.");
      setSubject("");
      setMessage("");
    } catch (err) {
      notify.fromError(err, "Gagal mengirim masukan");
    } finally {
      setSubmitting(false);
    }
  };
