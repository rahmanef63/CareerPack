...
export function FeedbackPanel() {
  ...
  const columns: ReadonlyArray<ColumnDef<Row>> = [
    ...
    },
    {
      id: "submitter",
      header: "Dari",
      // The feedback form lives behind the dashboard auth guard, so a row
      // with no email is a signed-in account that has none — i.e. a guest
      // session — not an unidentified walk-in.
      accessor: (r) => r.submitterEmail ?? GUEST_LABEL,
      cell: (r) => (
        <span className="text-sm font-mono">{r.submitterEmail ?? <span className="italic text-muted-foreground">{GUEST_LABEL}</span>}</span>
      ),
      width: "w-[220px]",
      hideOnMobile: true,
    },
