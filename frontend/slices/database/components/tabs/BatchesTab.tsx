...
export function BatchesTab() {
  ...
  const handleUndo = async (id: Id<"quickFillBatches">) => {
    ...
    try {
      ...
      notify.success(`Impor dibatalkan — ${res.deleted} baris dihapus.`);
    } catch (err) {
    ...
    },
    {
      ...
      accessor: (r) =>
        [
          ...
          r.goalIds.length ? `${r.goalIds.length} target karir` : null,
    ...
    },
    {
      ...
      cell: (r) => (
        ...
        >
          ...
          {r.undone ? "Sudah" : pendingId === r._id ? "Memproses…" : "Batalkan"}
        </Button>
      ),
    },
  ];
  ...
  return (
    <DataTable
      ...
      searchPlaceholder="Cari riwayat impor…"
      isLoading={data === undefined}
      emptyMessage="Belum ada riwayat Isi Cepat."
    />
  );
}
