...
export function AddJobDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      notify.success("Lowongan terbaca — periksa lalu simpan");
    } catch (err) {
      notify.fromError(err, "Gagal membaca lowongan");
    } finally {
      setParsing(false);
    }
  };
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <Textarea
              id="paste"
              rows={5}
              placeholder="Tempel deskripsi lowongan di sini (judul + perusahaan + tanggung jawab + kualifikasi)…"
              value={pasteText}
          ...
          <div className="border-t border-border pt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              />
              <Field label="Gaji min" id="salaryMin" type="number" value={form.salaryMin} onChange={(v) => setForm({ ...form, salaryMin: v })} />
              <Field label="Gaji maks" id="salaryMax" type="number" value={form.salaryMax} onChange={(v) => setForm({ ...form, salaryMax: v })} />
              <Field label="Mata uang" id="currency" placeholder="cth. IDR / USD" value={form.currency} onChange={(v) => setForm({ ...form, currency: v })} />
              <Field label="Link lamaran" id="applyUrl" value={form.applyUrl} onChange={(v) => setForm({ ...form, applyUrl: v })} />
            </div>
            <div className="mt-3 space-y-2">
              <Label htmlFor="skills">Skill (pisahkan dengan koma)</Label>
              <Input
                id="skills"
