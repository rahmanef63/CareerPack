...
import {
  Plus, Sparkles, Search, X, Trash2, CheckSquare, Library, Folder,
} from "lucide-react";
import Link from "next/link";
import { notify } from "@/shared/lib/notify";

import { EmptyState } from "@/shared/components/feedback/EmptyState";
import { NoPortfolio } from "@/shared/components/illustrations/empty";
    ...
    <PageContainer size="lg" className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {[
          ...
          { label: "Sertifikasi", value: stats.certification ?? 0, tint: "text-success-text" },
        ].map((s) => (
      ...
      <Tabs value={filter} onValueChange={(v) => setFilter(v as PortfolioFilter)}>
        ...
        <TabsContent value={filter} className="mt-4">
          {isLoading ? (
            ...
            <EmptyState
              // ponytail: art only on the all-tab — on a category tab an
              // illustration reads as breakage, not as "kategori ini kosong".
              illustration={filter === "all" ? NoPortfolio : undefined}
              icon={filter === "all" ? undefined : Folder}
              title={`Belum ada ${
                filter === "all"
                  ? "portofolio"
