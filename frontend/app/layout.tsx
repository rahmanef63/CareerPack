...
export const metadata: Metadata = {
  ...
  // The share card is the ONE surface where Indonesian copy is the wrong call,
  // and it is the only surface some people ever see. `og-card.jpg` already
  // renders its headline in English ("Everything you need to build your
  // career."), so Indonesian tags under it read as half-finished on every
  // unfurl — a Product Hunt preview, a launch tweet, a Slack or LinkedIn
  // reshare. The Google Translate picker cannot rescue this: OG tags are
  // scraped server-side, with no browser and no cookie.
  //
  // `locale` deliberately stays `id_ID`, and `metadata.title.default` /
  // `description` above deliberately stay Indonesian — those drive the browser
  // tab and Indonesian search results, where the actual users are. Only the
  // strings that travel with the English artwork are English.
  //
  // `og-card.jpg` is rendered from `scripts/og-card.html` (that file carries the
  // command). Its predecessor `og-light.jpg` was a sourceless flat JPEG whose
  // mockup rows had aged into "2 May 2024"; nothing in the replacement is
  // dated, so it never needs a re-render for a reason other than a brand or
  // feature change.
  openGraph: {
    type: "website",
    locale: "id_ID",
    url: SITE_URL,
    siteName: "CareerPack",
    title: "CareerPack — Everything you need to build your career",
    description:
      "CV builder, job-application tracker, skill roadmaps, mock interviews and an AI career assistant. Built in Bahasa Indonesia — one tap translates the whole app.",
    images: [
      {
        url: "/og/og-card.jpg",
        width: 1200,
        height: 630,
        alt: "CareerPack — everything you need to build your career: CV builder, job application tracker, skill roadmaps, mock interviews, AI career assistant",
      },
    ],
  },
  twitter: {
